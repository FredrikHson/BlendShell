    ...
    def execute(self, context):
        ...
        for it in range(0,itrs):
            ...
            if bstool.bs_usemods:
                ...
                bpy.context.object.modifiers["Shrinkwrap"].project_limit=thickness*1.1
                bpy.context.object.modifiers["Shrinkwrap"].use_negative_direction=True
        ...
        #'''
            # if bstool.bs_remesh !=0 and (it%bstool.bs_remesh) == 0 and it != itrs and it != 0:
                ...
                # if bpy.app.version[1] > 89 or bpy.app.version[0] >= 3:
                    # bpy.ops.object.modifier_apply(modifier="Remesh")
                # else:
...
class CCProperties(PropertyGroup):
    ...
    bs_dv: FloatProperty(
            name = "Expansion Step",
            description = "Percentage of min thickness as step size for each iteration of seed expansion",
            default = 0.1,
            precision = 6,
            step=1,
            unit= 'LENGTH',
